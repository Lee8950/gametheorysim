...
std::uniform_real_distribution<> wdist(0.0, 30000.0);
...
double ecl::get_alpha(int k, double Q, std::vector<ecl::File> &files, std::vector<ecl::Device> &devices) {
    int s1c = S1Count(k, devices);
    double x = S1Count(k, devices) * Q / Dk(k, files, devices);
    double demanding = Dk(k, files, devices);
    if (k < 4 && ecl::debug_var == 0)
        auto a = 1;
    return std::min(S1Count(k, devices) * Q / Dk(k, files, devices), static_cast<double>(1.0));
}
